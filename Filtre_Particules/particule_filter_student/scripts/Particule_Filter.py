...
class Particule_Filter:
    ...
    def getRandParticule(self, nbr, start_x, max_x, start_y, max_y):
        ...
        for i in range(1, nbr):
            particule_list.append(Particule(random.randrange(start_x, max_x, 1), self.FIXED_PLANE_Y, 5, 0))
    ...
    def motion_prediction(self):
        ...
        for i in range(len(self.particule_list)):
            ...
            new_particule_list.append(Particule(x_coord + 1, y_coord, 0, 0))
        ...
        #  Note that weighted_random_choice return a string containing coordinate x and y of the selected particules
    ...
    def weightingParticule(self,p_x, p_y, observed_distance):
        ...
        distance = distance_to_obstacle(p_x, p_y, self.obs_grid, self.width, self.height, self.SCALE_FACTOR)

        diff = observed_distance - distance

        ecart_type = 1.0

        weight = (1.0/(ecart_type*math.sqrt(2*math.pi)))*math.exp(-diff/(2*ecart_type))
